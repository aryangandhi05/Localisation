...
def helmert_transformation(Xloc, Xglo):
    ...
    # summation
    summation_Rm = np.sum(Rm, axis=1)
    print("Summation of Rm rows:", summation_Rm)
    # Handle potential non-convergence of SVD
    try:
        RTm_Rm = np.dot(Rm.T, Rm)
        print("RTm_Rm matrix:")
        print(RTm_Rm)
        U, _, Vt = np.linalg.svd(RTm_Rm)
...
def demo():
    ...
    plt.subplot(1, 3, 3)
    plt.title('Transformed Out-of-sample Data')
